...
pub fn create_party(conn: &PgConnection, new_name: &str, new_phone: &str, new_eban: &str, new_address_id: i32) -> QueryResult<Party> {
...
pub fn get_party_by_id(conn: &PgConnection, party_id: i32) -> QueryResult<Party> {
    parties.filter(id.eq(party_id)).first(conn)
}

pub fn delete_party(conn: &PgConnection, party_id: i32) -> QueryResult<usize> {
